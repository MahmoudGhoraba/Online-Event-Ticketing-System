...
import EventList from "../eventComponents/EventList"; // Adjust path if needed
...
export default function AdminUsersPage() {
  ...
  const currentUserRole = "Admin"; // or "Organizer", "User", etc.
  ...
  const updateEventStatus = async (event, newStatus) => {
    try {
      ...
      setEvents((prevEvents) =>
        prevEvents.map((evt) =>
          evt._id === event._id ? res.data.event : evt
        )
      );
  ...
  return (
    ...
    >
      <h1 style={{ color: "#2563EB" }}>Admin Users</h1>
